...
`DS4_CUDA_TP_PREFILL_ATTN_ROWS=1` selects the evidence-gated SM75 query-row
experiment for the complete ratio-4 indexed score -> top-k -> attention chain.
It incrementally mirrors raw, compressed-attention, and indexer caches on the
NVLink partner; transfers the latter half of the indexer query, indexer
weights, and attention query; executes both 50/50 chains concurrently; and
gathers the partner's full-head result before the unchanged inverse-RoPE/output
projection. The indexed scope is fail-closed: a requested call that cannot use
bidirectional peer access, all three partner-local mirrors, or the exact 50/50
shape aborts instead of silently running on the home GPU. Mixed, static-mixed,
and raw-only attention remain unchanged because a transfer-inclusive win was
not established for those paths. Set
...
The complete source-reviewed SM80-to-SM75 dispatch and resource baseline is in
[`CUDA_SM80_TO_SM75_DISPATCH_MATRIX.md`](CUDA_SM80_TO_SM75_DISPATCH_MATRIX.md).
Its paired cubin audit is generated by
