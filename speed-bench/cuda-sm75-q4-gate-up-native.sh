...
profile_launch_skips=(1 1 1 1 1 1 2)
profile_kernels=(
...
(( 10#$BENCH_ROUNDS % ${#benchmark_variants[@]} == 0 )) ||
    die "BENCH_ROUNDS must be a multiple of 10 to balance ten variants across sample positions"
...
if [[ $RESUME_NCU == 1 ]]; then
    for required_file in "$OUTPUT_DIR/elf-list.txt" "$OUTPUT_DIR/sass.txt" \
            ...
            "$OUTPUT_DIR/sass-eligibility.csv"; do
    ...
    python3 - "$OUTPUT_DIR/sass-summary.csv" \
            "$OUTPUT_DIR/sass-eligibility.csv" <<'PY'
import csv
import sys

path, eligibility_path = sys.argv[1:]
...
native_labels = ("native-w", "native-aw", "native-aw-warp16", "native-aw-stage7")
...
consumer_labels = ("standard", "standard-warp16", *native_labels)
with open(eligibility_path, newline="", encoding="utf-8-sig") as handle:
    eligibility = {row["label"]: row for row in csv.DictReader(handle)}
if set(eligibility) != set(consumer_labels):
    raise SystemExit(
        f"unexpected SASS eligibility labels: {sorted(eligibility)}"
    )
for label in consumer_labels:
    ...
    try:
        recorded_ldl = int(row["ldl"])
        recorded_stl = int(row["stl"])
    except (KeyError, ValueError) as error:
        raise SystemExit(f"invalid SASS eligibility {label}: {error}")
    expected = "true" if ldl == 0 and stl == 0 else "false"
    if (recorded_ldl, recorded_stl) != (ldl, stl):
        raise SystemExit(f"SASS eligibility counts disagree for {label}")
    if row.get("local_memory_free") != expected:
        raise SystemExit(f"SASS eligibility status disagrees for {label}")
PY
    ...
    printf 'label,ldl,stl,local_memory_free\n' \
        >"$OUTPUT_DIR/sass-eligibility.csv"
    ...
    if [[ $label == standard || $label == standard-warp16 ]]; then
        (( imma_8816 > 0 )) || die "$kernel contains no 8x8x16 IMMA instruction"
            ...
            $label == native-aw-warp16 || $label == native-aw-stage7 ]]; then
    ...
    if [[ $label == standard || $label == standard-warp16 ||
        ...
        printf '%s,%s,%s,%s\n' "$label" "$ldl" "$stl" \
            "$local_memory_free" >>"$OUTPUT_DIR/sass-eligibility.csv"
    fi
    done
fi
...
validate_benchmark() {
    ...
    grep -Fxq 'benchmark_position_balance_period=10' "$log" ||
        die "$scenario benchmark omitted ten-way position-balance evidence"
