...
QUALITY_SCORE_OBJ := gguf-tools/quality-testing/score_official.o
...
$(QUALITY_SCORE_OBJ): gguf-tools/quality-testing/score_official.c ds4.h ds4_gpu.h ds4_gpu_args.h ds4_gpu_mgpu.h ds4_ssd.h
	$(CC) $(QUALITY_CFLAGS) -I. -c -o $@ $<

ds4.o: ds4.c ds4.h ds4_ssd.h ds4_distributed.h ds4_gpu.h ds4_gpu_mgpu.h ds4_tensor_layout.h
	$(CC) $(CFLAGS) -c -o $@ ds4.c
...
ds4_bench.o: ds4_bench.c ds4.h ds4_ssd.h ds4_distributed.h ds4_help.h ds4_gpu.h ds4_gpu_args.h ds4_gpu_mgpu.h
...
tests/cuda_sm75_decode_weight_profile.o: tests/cuda_sm75_decode_weight_profile.c ds4_gpu.h ds4_tensor_layout.h
	$(CC) $(CFLAGS) -I. -c -o $@ tests/cuda_sm75_decode_weight_profile.c

tests/cuda_sm75_compressor_state_fusion.o: tests/cuda_sm75_compressor_state_fusion.c ds4_gpu.h
	$(CC) $(CFLAGS) -I. -c -o $@ tests/cuda_sm75_compressor_state_fusion.c

tests/cuda_sm75_t32_decode_f16_ab.o: tests/cuda_sm75_t32_decode_f16_ab.c ds4_gpu.h
	$(CC) $(CFLAGS) -I. -c -o $@ tests/cuda_sm75_t32_decode_f16_ab.c
...
tests/cuda_sm75_token_row_arithmetic.o: tests/cuda_sm75_token_row_arithmetic.c ds4_gpu.h
	$(CC) $(CFLAGS) -I. -c -o $@ tests/cuda_sm75_token_row_arithmetic.c
...
ds4_cpu.o: ds4.c ds4.h ds4_ssd.h ds4_distributed.h ds4_gpu.h ds4_gpu_mgpu.h
...
ds4_bench_cpu.o: ds4_bench.c ds4.h ds4_ssd.h ds4_distributed.h ds4_help.h ds4_gpu.h ds4_gpu_args.h ds4_gpu_mgpu.h
...
ds4_cuda.o: ds4_cuda.cu ds4_gpu.h ds4_gpu_mgpu.h ds4_tensor_layout.h ds4_iq2_tables_cuda.inc ds4_cuda_sm75_q4_native.inc.cu ds4_cuda_sm75_q32_native.inc.cu
...
tests/cuda_sm75_decode_weight_profile: tests/cuda_sm75_decode_weight_profile.o ds4_cuda.o
	$(NVCC) $(NVCCFLAGS) -o $@ $^ $(CUDA_LDLIBS)

tests/cuda_sm75_compressor_state_fusion: tests/cuda_sm75_compressor_state_fusion.o ds4_cuda.o
	$(NVCC) $(NVCCFLAGS) -o $@ $^ $(CUDA_LDLIBS)

tests/cuda_sm75_t32_decode_f16_ab: tests/cuda_sm75_t32_decode_f16_ab.o ds4_cuda.o
...
tests/cuda_sm75_token_row_arithmetic: tests/cuda_sm75_token_row_arithmetic.o ds4_cuda.o
	@test "$(CUDA_ARCH)" = "sm_75" || { echo "error: $@ requires CUDA_ARCH=sm_75" >&2; exit 1; }
	$(NVCC) $(NVCCFLAGS) -o $@ $^ $(CUDA_LDLIBS)
...
tests/cuda_sm75_q3_q4_32: tests/cuda_sm75_q3_q4_32.cu
	@test "$(CUDA_ARCH)" = "sm_75" || { echo "error: $@ requires CUDA_ARCH=sm_75" >&2; exit 1; }
	$(NVCC) $(filter-out --use_fast_math,$(NVCCFLAGS)) -Xptxas -v -o $@ $< $(CUDA_LDLIBS)
...
tests/cuda_sm75_q8_warp_interleaved: tests/cuda_sm75_q8_warp_interleaved.cu
	@test "$(CUDA_ARCH)" = "sm_75" || { echo "error: $@ requires CUDA_ARCH=sm_75" >&2; exit 1; }
	$(NVCC) $(NVCCFLAGS) -Xptxas -v -o $@ $< $(CUDA_LDLIBS)
...
ds4_cpu_test_hooks.o: ds4.c ds4.h ds4_gpu.h ds4_gpu_mgpu.h ds4_layer_pack.h ds4_tensor_layout.h
...
tests/cuda_sm75_pair_stability: tests/cuda_sm75_pair_stability.cu
	@test "$(CUDA_ARCH)" = "sm_75" || { echo "error: $@ requires CUDA_ARCH=sm_75" >&2; exit 1; }
	$(NVCC) $(NVCCFLAGS) -o $@ $< $(CUDA_LDLIBS)

tests/cuda_device_identity: tests/cuda_device_identity.cu
	$(NVCC) $(NVCCFLAGS) -o $@ $< $(CUDA_LDLIBS)
