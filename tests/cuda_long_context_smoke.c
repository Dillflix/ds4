...
static int check_compact_decode_zero_compressed_rows(void) {
    const uint32_t n_head = 8u;
    const uint32_t head_dim = 512u;
    const uint32_t n_raw = 4u;
    ...
    if (!sinks || !q_host || !raw_host || !control_host || !compact_host) {
        free(compact_host);
        free(control_host);
        free(raw_host);
        free(q_host);
    ...
    for (uint64_t i = 0; i < head_count; i++) {
        q_host[i] = (float)((int)(i % 29u) - 14) * 0.00390625f;
    }
    for (uint64_t i = 0; i < raw_count; i++) {
        raw_host[i] = (float)((int)(i % 31u) - 15) * 0.0078125f;
    }
    ...
    if (control && candidate && q && raw &&
        ...
        ds4_gpu_attention_decode_heads_tensor(
            ...
            n_raw, n_raw, 0, NULL, DS4_GPU_ATTN_COMP_CACHE_F32, 0,
        ...
        ds4_gpu_attention_decode_heads_tensor(
            ...
            n_raw, n_raw, 0, NULL,
            DS4_GPU_ATTN_COMP_CACHE_SM75_COMPACT, 0,
            NULL, 0, n_head, head_dim) &&
        ds4_gpu_synchronize() &&
        ...
        if (memcmp(control_host, compact_host,
            ...
            fprintf(stderr,
                    "cuda-regression: compact-format decode with zero "
                    "compressed rows uses raw-only path exactly\n");
            rc = 0;
        } else {
            fprintf(stderr,
                    "compact-format zero-row decode differs from raw-only "
                    "control\n");
        }
    }
...
static int check_compact_token_tiled_prefill_exact(int indexed) {
    const uint32_t n_tokens = 65u, pos0 = 4096u;
    const uint32_t n_head = 16u, head_dim = 512u, n_rot = 64u;
    const uint32_t n_raw = 128u, raw_cap = 160u, raw_start = 19u;
    const uint32_t n_comp = indexed ? 1041u : 1024u, top_k = 512u;
    const uint32_t window = 128u, ratio = 4u;
    ...
    int rc = 1;
    (void)unsetenv("DS4_CUDA_NO_ATTN_COMPACT_HYBRID");
    if (control && candidate && q && raw && comp_source && comp_f32 &&
        comp_compact && topk &&
        ds4_gpu_tensor_write(q, 0, q_host, head_count * sizeof(float)) &&
        ds4_gpu_tensor_write(raw, 0, raw_host, raw_count * sizeof(float)) &&
        ...
        (indexed
            ? ds4_gpu_attention_indexed_mixed_batch_heads_tensor(
                control, sinks, n_head * sizeof(float), 0u, q, raw,
                comp_f32, DS4_GPU_ATTN_COMP_CACHE_F32, topk, n_tokens,
                pos0, n_raw, raw_cap, raw_start, n_comp, top_k, window,
                ratio, n_head, head_dim)
            : ds4_gpu_attention_decode_mixed_batch_heads_tensor(
                control, sinks, n_head * sizeof(float), 0u, q, raw,
                comp_f32, DS4_GPU_ATTN_COMP_CACHE_F32, NULL, 0u, n_tokens,
        ...
        (indexed
            ...
            : ds4_gpu_attention_decode_mixed_batch_heads_tensor(
                candidate, sinks, n_head * sizeof(float), 0u, q, raw,
                comp_compact, DS4_GPU_ATTN_COMP_CACHE_SM75_COMPACT, NULL,
                0u, n_tokens, pos0, n_raw, raw_cap, raw_start, n_comp,
                window, ratio, n_head, head_dim)) &&
        ds4_gpu_synchronize() &&
        ds4_gpu_tensor_read(candidate, 0, compact_host,
                            head_count * sizeof(float))) {
        if (memcmp(control_host, compact_host,
                   (size_t)head_count * sizeof(float)) == 0) {
            fprintf(stderr,
                    "cuda-regression: compact %s 65-token prefill tiles "
                    "32/32/1 through double-buffered H16 exactly\n",
            ...
            while (first < head_count &&
                   memcmp(control_host + first, compact_host + first,
                          sizeof(float)) == 0) first++;
            fprintf(stderr,
                    "compact %s token-tiled prefill differs from shipping "
                    "F32 at %llu\n", indexed ? "indexed" : "dense",
                    (unsigned long long)first);
        }
    }
...
int main(void) {
    ...
    if (check_compact_token_tiled_prefill_exact(0) != 0) rc = 1;
    if (check_compact_token_tiled_prefill_exact(1) != 0) rc = 1;
